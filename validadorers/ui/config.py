...
class UserConfig(BaseDialog):
    ...
    def __add_option_color(self, idx, key, layout):
        ...
        # Load selected option (convert to string if needed)
        value = self.user_setting[self.key_name][key]
        editor.setText(str(value) if not isinstance(value, str) else value)
        # Context menu
        editor.defaults = self.default_setting[self.key_name][key]
        add_context_menu(editor)
        # Add layout
