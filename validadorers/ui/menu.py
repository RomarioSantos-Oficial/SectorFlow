...
class OverlayMenu(QMenu):
    ...
    def __init__(self, title, parent, is_tray: bool = False):
        ...
        # Lock overlay
        self.overlay_lock = self.addAction(_("lock_overlay"))
        self.overlay_lock.setCheckable(True)
        self.overlay_lock.triggered.connect(self.is_locked)

        # Auto hide
        self.overlay_hide = self.addAction(_("auto_hide"))
        self.overlay_hide.setCheckable(True)
        self.overlay_hide.triggered.connect(self.is_hidden)

        # Grid move
        self.overlay_grid = self.addAction(_("grid_move"))
        self.overlay_grid.setCheckable(True)
        self.overlay_grid.triggered.connect(self.has_grid)

        # VR Compatbiility
        self.overlay_vr = self.addAction(_("vr_compatibility"))
        self.overlay_vr.setCheckable(True)
        self.overlay_vr.triggered.connect(self.vr_compatibility)

        # Reload preset
        self.reload_preset = self.addAction(_("reload"))
        self.reload_preset.triggered.connect(parent.reload_preset)
        self.addSeparator()

        # Restart API
        self.restart_api = self.addAction(_("restart_api"))
        self.restart_api.triggered.connect(parent.restart_api)
        self.addSeparator()

        # Reset submenu
        menu_reset_data = ResetDataMenu(_("reset_data"), parent)
        self.addMenu(menu_reset_data)
        self.addSeparator()

        # Config
        if is_tray:
            self.app_config = self.addAction(_("config"))
