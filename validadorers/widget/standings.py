...
class Realtime(Overlay):
    ...
    def __init__(self, config, widget_name):
        ...
        # Ensure brand logo is enabled and positioned correctly
        self.wcfg["show_brand_logo"] = True
        
        # Layout: Position(1) -> Logo(2) -> Class(3) -> Driver(4)
        self.wcfg["column_index_position"] = 1
        self.wcfg["column_index_brand_logo"] = 2
        self.wcfg["column_index_class"] = 3
        self.wcfg["column_index_driver"] = 4
        
        # Shift other columns to avoid overlap
        if self.wcfg.get("column_index_laptime", 0) <= 4:
             self.wcfg["column_index_laptime"] = 5
        if self.wcfg.get("column_index_position_in_class", 0) <= 4:
             self.wcfg["column_index_position_in_class"] = 6
        if self.wcfg.get("column_index_vehicle", 0) <= 4:
             self.wcfg["column_index_vehicle"] = 7
        # ----------------------------------
    ...
    def set_brand_logo(self, brand_name: str):
        ...
        if brand_name not in self.pixmap_brandlogo:  # load & cache logo
            # Find matching logo
            logo_name = brand_name
            logo_ext = ".png"
            
            # 1. Check exact match (case insensitive)
            if brand_name.lower() in self.available_logos:
                logo_name, logo_ext = self.available_logos[brand_name.lower()]
            else:
                # 2. Check substring match
                # Sort available logos by length descending to match longest possible name first
                # e.g. "Aston Martin" before "Aston"
            ...
            self.pixmap_brandlogo[brand_name] = load_brand_logo_file(
                filepath=self.logo_search_path,
                filename=logo_name,
