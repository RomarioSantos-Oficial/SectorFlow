...
class Session(DataAdapter):
    ...
    def track_clock_time(self) -> float:
        """Track clock time (seconds since midnight)"""
...
class Vehicle(DataAdapter):
    ...
    def team_name(self, index: int | None = None) -> str:
        """Team name (Pit Group)"""
        return tostr(self.shmm.rf2ScorVeh(index).mPitGroup)
    ...
    def count_lap_flag(self, index: int | None = None) -> int:
    ...
    def virtual_energy(self, index: int | None = None) -> float:
        ...
        # Only available for player via REST API
        if self.shmm.rf2ScorVeh(index).mIsPlayer:
    ...
    def battery_charge_fraction(self, index: int | None = None) -> float:
        """Battery charge fraction [0.0-1.0]"""
        return rmnan(self.shmm.rf2TeleVeh(index).mBatteryChargeFraction)

    def electric_boost_motor_state(self, index: int | None = None) -> int:
        """Electric boost motor state: 0=unavailable 1=inactive, 2=propulsion, 3=regeneration"""
        return self.shmm.rf2TeleVeh(index).mElectricBoostMotorState
    ...
    def front_tire_compound_name(self, index: int | None = None) -> str:
        """Front tire compound name"""
        return tostr(self.shmm.rf2TeleVeh(index).mFrontTireCompoundName)

    def rear_tire_compound_name(self, index: int | None = None) -> str:
        """Rear tire compound name"""
        return tostr(self.shmm.rf2TeleVeh(index).mRearTireCompoundName)
